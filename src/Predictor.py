...
2. 处理 Polars DataFrame（转换为 Pandas）
3. 计算衍生特征
4. 使用 GPU（如果可用）
...
from model import TKANPro, create_model
...
class Predictor:
    ...
    def __init__(self):
        ...
        self.model = create_model(input_dim=input_dim, num_windows=5, dropout=0.0)
        self.model.load_state_dict(checkpoint['model_state'])
        self.model.to(self.device)
        self.model.eval()
        
        print(f"模型加载成功，最佳窗口: {self.best_window}")
    
    def preprocess(self, x: List[pd.DataFrame]) -> torch.Tensor:
        """预处理输入数据
        
        Args:
            x: List[pd.DataFrame]，长度为 batch，每个 DataFrame 为 100 tick 数据
...
if __name__ == '__main__':
    ...
    test_data = [pd.DataFrame({
        'bid1': np.random.randn(100) * 0.01,
        'ask1': np.random.randn(100) * 0.01,
        'bsize1': np.random.rand(100) * 10,
        'asize1': np.random.rand(100) * 10,
    ...
    for i in range(2):
        for j in range(2, 11):
            test_data[i][f'bid{j}'] = np.random.randn(100) * 0.01
            test_data[i][f'ask{j}'] = np.random.randn(100) * 0.01
            test_data[i][f'bsize{j}'] = np.random.rand(100) * 10
            test_data[i][f'asize{j}'] = np.random.rand(100) * 10
