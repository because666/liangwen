...
from models.model import DeepProfitNet, create_model, count_parameters
from training.losses import CompositeLoss, compute_trading_metrics
...
# 异常值处理参数
FEATURE_CLIP_RANGE = {
    'bid': (-0.3, 0.3),      # 价格涨跌幅限制 ±30%
    'ask': (-0.3, 0.3),
    'bsize': (0, 100),       # 换手率限制
    'asize': (0, 100),
    'intst': (-100, 100),    # 订单强度限制
    'midprice': (-0.3, 0.3),
    'imbalance': (-1, 1),
    'cumspread': (-1, 1),
    'ofi': (-100, 100),
}
RETURN_CLIP_RANGE = (-0.1, 0.1)  # 收益率限制 ±10%
...
def clean_returns(returns: np.ndarray) -> np.ndarray:
    """清洗收益率数据：处理 NaN、Inf 和极端值"""
    returns = returns.copy()
    returns = np.nan_to_num(returns, nan=0.0, posinf=0.0, neginf=0.0)
    returns = np.clip(returns, -0.1, 0.1)  # 限制在 ±10%
    return returns
    ...
    def __iter__(self):
        ...
        for file in files:
            ...
            try:
                ...
                true_returns = np.zeros((len(df), len(windows)), dtype=np.float32)
                for i, w in enumerate(windows):
                    if w < len(df):
                        # 防止除零：midprice 为 0 时设为小值
...
def train_epoch(model, dataloader, criterion, optimizer, device, scaler=None,
                max_grad_norm: float = 1.0, max_batches: int = None, 
                current_lr: float = None, warmup_lr: float = None):
    model.train()
    total_loss = 0
    total_ce = 0
    total_sharpe = 0
    ...
    for batch_idx, (features, labels, true_returns) in enumerate(pbar):
        ...
        loss_dict = criterion(logits, labels, true_returns, list(model.parameters()))
        loss = loss_dict['loss']

        # NaN 检测：如果出现 NaN，跳过当前 batch
        if torch.isnan(loss) or torch.isinf(loss):
            nan_count += 1
            if nan_count <= 3:
                print(f"\n[警告] batch {batch_idx} 出现 NaN/Inf，跳过此 batch (第{nan_count}次)")
                print(f"        ce_loss={loss_dict['ce_loss']:.4f}, "
                      f"sharpe_loss={loss_dict['sharpe_loss']:.4f}")
                optimizer.zero_grad()
                continue
            else:
        ...
        total_sharpe += loss_dict['sharpe_loss']
        total_sparse = loss_dict.get('sparse_loss', 0.0)
        num_batches += 1
        ...
        if batch_idx % 100 == 0:
            ...
            print(f"\n[调试] batch {batch_idx}: loss={loss.item():.4f}, "
                  ...
                  f"sharpe={loss_dict['sharpe_loss']:.4f}, "
                  f"sparse={total_sparse:.4f}, "
        ...
        pbar.set_postfix({
            ...
            'sparse': f'{total_sparse:.3f}',
        })

    return {
        'loss': total_loss / num_batches if num_batches > 0 else 0,
        'ce_loss': total_ce / num_batches if num_batches > 0 else 0,
        'sharpe_loss': total_sharpe / num_batches if num_batches > 0 else 0,
    }
...
def evaluate(model, dataloader, criterion, device, max_batches: int = None):
    ...
    with torch.no_grad():
        for batch_idx, (features, labels, true_returns) in enumerate(tqdm(dataloader, desc="评估中")):
            ...
            logits = model(features)
            loss_dict = criterion(logits, labels, true_returns)
...
def main():
    parser = argparse.ArgumentParser(description='深度收益导向分层集成网络训练')
    parser.add_argument('--data_dir', type=str,
                        default='/mnt/workspace/TEMP-FILE-STATION/良文杯/2026train_set/2026train_set')
    parser.add_argument('--output_dir', type=str,
                        default='/mnt/workspace/TEMP-FILE-STATION/良文杯/架构V3_深度收益导向/submission')
    parser.add_argument('--batch_size', type=int, default=256)
    parser.add_argument('--epochs', type=int, default=20)
    parser.add_argument('--lr', type=float, default=5e-5)  # 进一步降低学习率
    parser.add_argument('--weight_decay', type=float, default=1e-4)
    parser.add_argument('--max_grad_norm', type=float, default=0.3)  # 更严格的梯度裁剪
    ...
    parser.add_argument('--hold_weight', type=float, default=1.5)  # 提高观望权重，鼓励观望
    parser.add_argument('--lambda_sharpe', type=float, default=0.01)  # 进一步降低夏普损失权重
    parser.add_argument('--lambda_sparse', type=float, default=0.01)  # 稀疏正则权重
    parser.add_argument('--ema_decay', type=float, default=0.999)
    parser.add_argument('--patience', type=int, default=5)
    parser.add_argument('--warmup_epochs', type=int, default=3)  # warm-up 阶段
    ...
    print("创建模型...")
    print("=" * 50)
    model = create_model(input_dim, dropout=0.2).to(device)  # 增加 dropout 防止过拟合
    ...
    # 混合精度 - 由于 PyTorch 版本兼容性问题，暂时禁用
    # 等 PyTorch 修复后再启用
    scaler = None
    if args.use_amp:
        print("警告: 混合精度训练在当前 PyTorch 版本中存在兼容性问题，已自动禁用")
        print("使用全精度训练...")
        args.use_amp = False
    ...
    for epoch in range(args.epochs):
        ...
        if epoch < args.warmup_epochs:
            lr_mult = get_lr_multiplier(epoch, args.warmup_epochs)
        ...
        # 只在 warm-up 结束后才使用 cosine annealing
        if epoch >= args.warmup_epochs:
            scheduler.step()
        ...
        if val_metrics['cumulative_return'] > best_val_return:
            ...
            print(f"保存最佳模型, Epoch {epoch + 1}, 验证收益: {best_val_return:.6f}")
        else:
            patience_counter += 1
