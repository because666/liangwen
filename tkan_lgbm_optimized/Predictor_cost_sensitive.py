...
4. T-KAN 编码器提取 128 维特征 → 代价敏感 LightGBM 预测
...
class Predictor:
    ...
    def __init__(self):
        ...
        self.cost_sensitive_mode = checkpoint.get('cost_sensitive_mode', False)
        ...
        print(f"代价敏感 LightGBM 模型加载成功: {len(self.lgbm_models)} 个窗口")

        self.thresholds = checkpoint.get('thresholds', {w: 0.6 for w in WINDOW_SIZES})
        print(f"预测阈值: {self.thresholds}")
