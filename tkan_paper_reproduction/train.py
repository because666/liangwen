...
训练策略 (参考论文 Section IV-D):
1. Warm-up: 5 epochs, lr: 1e-5 -> 3e-5
2. 主训练: CosineAnnealing, lr: 3e-5 -> 1e-5
3. 早停: patience=5
4. EMA权重平滑: decay=0.999
5. 梯度裁剪: max_grad_norm=0.3

数据预处理 (参考论文 Section IV-B):
1. 两阶段缩放: 移动中位数 + MinMax
2. 特征清洗: NaN/Inf/异常值处理
3. 衍生特征计算: OFI/imbalance/cumspread
"""
...
from model import TKANModel, create_model, count_parameters
...
def main():
    parser = argparse.ArgumentParser(description='TKAN 论文复现 - 训练脚本')
    parser.add_argument('--data_dir', type=str,
                        default='d:/量化/良文杯/2026train_set/2026train_set')
    parser.add_argument('--output_dir', type=str,
                        default='d:/量化/良文杯/tkan_paper_reproduction')
    ...
    parser.add_argument('--num_tkan_layers', type=int, default=2)
    ...
    print("\n" + "=" * 50)
    print("创建 TKAN 模型 (论文复现)...")
    print("=" * 50)
    model = create_model(
        input_dim=input_dim,
        num_windows=5,
        hidden_dim=args.hidden_dim,
        num_tkan_layers=args.num_tkan_layers,
    ...
    for epoch in range(args.epochs):
        ...
        if epoch < args.warmup_epochs:
            ...
            print(f"Warm-up 阶段: 学习率 = {current_lr:.2e}")
        else:
